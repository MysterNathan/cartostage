...
func (r *userRepository) GetAll(ctx context.Context, filter models.UserFilter) ([]models.User, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get student-service: %w", err)
	}
...
func (r *userRepository) buildQuery(filter models.UserFilter) (string, []interface{}) {
	...
	// ⭐ Condition commune : on ne veut QUE des élèves
	conditions = append(conditions, "users.role = 'eleve'")

	switch filter.RequestorRole {
	case models.RoleAdmin:
		// Admin voit tous les élèves (pas de filtre supplémentaire)

	case models.RoleTeacher:
		// Teacher voit tous ses élèves
		joins = append(joins, "JOIN stages ON stages.student_id = users.id")
		conditions = append(conditions, fmt.Sprintf("stages.teacher_id = $%d", argPosition))
		args = append(args, filter.RequestorID)
		argPosition++

	case models.RoleStudent:
		// Student ne voit QUE lui-même (pas les autres élèves)
		conditions = append(conditions, fmt.Sprintf("users.id = $%d", argPosition))
		args = append(args, filter.RequestorID)
		argPosition++

	case models.RoleTutor:
		// Tutor voit uniquement les élèves qu'il encadre en stage-service
		joins = append(joins, "JOIN stages ON stages.student_id = users.id")
...
func (r *userRepository) GetByID(ctx context.Context, id int) (*models.User, error) {
	query := `SELECT * FROM users WHERE id = $1 AND role = 'eleve'`
...
func (r *userRepository) Create(ctx context.Context, user *models.User) error {
	// Force le role à 'student-service'
	user.Role = string(models.RoleStudent)
	...
	if err != nil {
		return fmt.Errorf("failed to create student-service: %w", err)
	}
...
func (r *userRepository) Update(ctx context.Context, user *models.User) error {
	...
	if err != nil {
		return fmt.Errorf("failed to update student-service: %w", err)
	}
	...
	if rows == 0 {
		return fmt.Errorf("student-service not found")
	}
...
func (r *userRepository) Delete(ctx context.Context, id int) error {
	query := `DELETE FROM users WHERE id = $1 AND role = 'eleve'`

	result, err := r.db.ExecContext(ctx, query, id)
	if err != nil {
		return fmt.Errorf("failed to delete eleve: %w", err)
	}
