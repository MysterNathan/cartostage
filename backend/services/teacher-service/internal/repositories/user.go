...
func (r *userRepository) buildQuery(filter models.UserFilter) (string, []interface{}) {
	...
	conditions = append(conditions, "users.role = 'teacher-service'")
...
func (r *userRepository) GetByID(ctx context.Context, id int) (*models.User, error) {
	query := `SELECT * FROM users WHERE id = $1 AND role = 'teacher-service'`

	var user models.User
	err := r.db.GetContext(ctx, &user, query, id)
	if err != nil {
		return nil, fmt.Errorf("failed to get teacher-service: %w", err)
	}
...
func (r *userRepository) Create(ctx context.Context, user *models.User) error {
	// Force le role à 'teacher-service'
	...
	if err != nil {
		return fmt.Errorf("failed to create teacher-service: %w", err)
	}
...
func (r *userRepository) Update(ctx context.Context, user *models.User) error {
	query := `
        ...
        WHERE id = $8 AND role = 'teacher-service'
    `
	...
	if err != nil {
		return fmt.Errorf("failed to update teacher-service: %w", err)
	}
	...
	if rows == 0 {
		return fmt.Errorf("teacher-service not found")
	}
...
func (r *userRepository) Delete(ctx context.Context, id int) error {
	query := `DELETE FROM users WHERE id = $1 AND role = 'teacher-service'`

	result, err := r.db.ExecContext(ctx, query, id)
	if err != nil {
		return fmt.Errorf("failed to delete teacher-service: %w", err)
	}
