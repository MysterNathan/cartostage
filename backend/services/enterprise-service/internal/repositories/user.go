...
func (r *userRepository) buildQuery(filter models.UserFilter) (string, []interface{}) {
	...
	// ⭐ Condition commune : on ne veut QUE des tuteurs
	conditions = append(conditions, "users.role = 'tutor'")

	switch filter.RequestorRole {
	case models.RoleAdmin:
		// Admin voit tous les tuteurs (pas de filtre supplémentaire)

	case models.RoleTeacher:
		// Teacher voit les tuteurs de ses élèves
		joins = append(joins, "JOIN stages ON stages.tutor_id = users.id")
		conditions = append(conditions, fmt.Sprintf("stages.teacher_id = $%d", argPosition))
		args = append(args, filter.RequestorID)
		argPosition++

	case models.RoleStudent:
		// Student voit son tuteur
		joins = append(joins, "JOIN stages ON stages.tutor_id = users.id")
	...
	case models.RoleTutor:
		// Tutor voit les autres tuteurs de son entreprise
		subQuery := fmt.Sprintf("(SELECT establishment_id FROM users WHERE id = $%d)", argPosition)
		conditions = append(conditions, fmt.Sprintf("users.establishment_id = %s", subQuery))
		args = append(args, filter.RequestorID)
		argPosition++
	}
...
func (r *userRepository) Update(ctx context.Context, user *models.User) error {
	query := `
        ...
        SET username = $1, first_name = $2, last_name = $3, email = $4, 
            role = $5, phone = $6, establishment_id = $7, is_active = $8, updated_at = CURRENT_TIMESTAMP
        WHERE id = $9
    `
